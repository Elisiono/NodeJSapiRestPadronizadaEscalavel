...
roteador.post('/', async (req,res, proximo)=> {
    try{
        ...
        const empresa = "Gatito"
        res.set('X-Powered-By', empresa)
        res.status(201)
        res.send(
...
roteador.options('/:id', (req, res)=>{
    res.set('Access-Control-Allow-Methods', 'DELET, GET, HEAD, PUT')
    res.set('Access-Control-Allow-Headers', 'Content-Type')
    res.status(204)
    res.end()
})
...
roteador.get('/:id', async (req,res, proximo)=> {
    try {
        ...
        const serializador = new Serializador(
            res.getHeader('Content-Type'),
            [ 'preco', 'estoque', 'fornecedor', 'dataCriacao', 'dataAtualizacao', 'dataAtualizacao', 'versao']
        )
